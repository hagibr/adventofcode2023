puzzle_input = '''FL7L-F7FJF-7FFF7FF|-F-F|JF--|.|-J-7.F--FJF|-7.F.-FF7.-F7-L-|FF-F-|-7.-F7-FF-|7J-J.F-J7.FF7..L7F|7F-7JF-L7F.|--FL7F---|.FFF-L7FL--L777L-.FF7F
...
for line_number, line in enumerate(maze):
  for col_number, col in enumerate(line):
    if( col == 'S'):
      start_position = (line_number, col_number)
      up_limit = down_limit = line_number
      left_limit = right_limit = col_number
      break
...
# Now, from the start we try to find a way to go through the loop
c_line, c_col = start_position
# This is a set that stores all the coordinates of tiles that are part of the loop
loop = {None}
# Just for 
...
while True:
  # Going UP: the last movement can't be DOWN / the candidate tile can't be already in the loop / we must be in a tile that allow going UP / the candidate tile must be one that allow coming from DOWN
  if last_movement != DOWN and (c_line-1,c_col) not in loop and maze[c_line][c_col] in ('S','|','L','J') and maze[c_line-1][c_col] in ('|','F','7','S'):
    c_line = c_line-1
    last_movement = UP
  # Going DOWN
  elif last_movement != UP and (c_line+1,c_col) not in loop and maze[c_line][c_col] in ('S','|','7','F') and maze[c_line+1][c_col] in ('|','J','L','S'):
    c_line = c_line+1
    last_movement = DOWN
  # Going LEFT
  elif last_movement != RIGHT and (c_line,c_col-1) not in loop and maze[c_line][c_col] in ('S','-','7','J') and maze[c_line][c_col-1] in ('-','L','F','S'):
    c_col = c_col-1
    last_movement = LEFT
  # Going RIGHT
  elif last_movement != LEFT and (c_line,c_col+1) not in loop and maze[c_line][c_col] in ('S','-','L','F') and maze[c_line][c_col+1] in ('-','7','J','S'):
    c_col = c_col+1
    last_movement = RIGHT
  # Nowhere to go??
  else:
    print(f"Stuck in position ({c_line},{c_col})")
    break
  # print((c_line, c_col), end = ';')
  # Now we add the new tile to the loop
  loop.add((c_line, c_col))
  # Updating the loop limits
  left_limit = min(c_col, left_limit)
  right_limit = max(c_col, right_limit)
  up_limit = min(c_line, up_limit)
  down_limit = max(c_line, down_limit)
  # Incrementing the loop count
  count_steps += 1
  # Checking if we have completed the loop
  if( (c_line, c_col) == start_position ):
    # print("End")
    break
  
# The "farthest" tile is at "half distance" of the starting tile
print(f"Part One: {count_steps/2}")

# There is no enclosed tiles outside the 4 limits
enclosed_count = 0
for l in range(up_limit, down_limit+1):
  for c in range(left_limit, right_limit+1):
    # If the current tile is not part of the loop...
    if( (l,c) not in loop ):
      cross_count = 0
      last_cross_type = ' '
      # Now we go to the right direction and count the number of crossing with a tile that is part of the loop
      for c2 in range(c+1,right_limit+1):
        if( (l,c2) in loop and maze[l][c2] in ('S','L','F','7','J','|') ):
          # Horizontal tile is always a valid crossing
          if( maze[l][c2] == '|' ):
            cross_count += 1
          # Checking if this is a crossing going diagonally down-left <-> up-right
          elif( last_cross_type == 'F' and maze[l][c2] == 'J' ):
            cross_count += 1
          # Checking if this is a crossing going diagonally up-left <-> down-right
          elif( last_cross_type in ('L','S') and maze[l][c2] == '7' ): # Start point is a 'L' type
            cross_count += 1
          last_cross_type = maze[l][c2]
      # Odd number of crossings means that we are enclosed by the loop
      if( cross_count % 2 != 0 ):
        enclosed_count += 1

print(f"Part Two: {enclosed_count}")
