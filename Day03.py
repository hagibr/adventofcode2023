...
sum_part = 0 # Answer for Part One
gear_map = {} # Used in Part Two
for line_idx, line in enumerate(schematic):
  skip_i = 0
  # First we search for the start of a number
  for i, c in enumerate(line):
    # skipping indexes of previous numbers
    if i < skip_i:
      continue
    # Found the start?
      ...
      number_end = i
      i = i + 1
      # Let's find the end of this number
      while( i < len(line) ):
        c = line[i]
        if( c in "0123456789" ):
          number_end = i
          ...
          break
      # Stores the quantity of index to skip in the next iteration
      skip_i = i
      
      # Defining the window of search for symbols in the schematic
      if( number_start > 0 ):
        window_start_x = number_start - 1
      else:
        window_start_x = 0
      if( number_end < len(line) - 2):
        window_end_x = number_end + 1
      else:
        window_end_x = number_end
      if( line_idx > 0):
        window_start_y = line_idx - 1
      else:
        window_start_y = 0
      if( line_idx < len(schematic) - 2):
        window_end_y = line_idx + 1
      else:
      ...
      for y in range(window_start_y, window_end_y+1):
        for x in range(window_start_x, window_end_x+1):
          if( schematic[y][x] not in "0123456789." ):
            
            # As we don't stop the window search, let's flag when we've already found
            # an adjacent symbol and added the part to the sum
            if( not found_part ):
              sum_part += part_number
              found_part = True
            
            # This part was added for using in part 2.
            if( schematic[y][x] == '*'):
              # We map every gear to a list of adjacent part numbers
              if( (x,y) not in gear_map):
                gear_map[(x,y)] = [part_number]
              else:
                gear_map[(x,y)].append(part_number)
            

print(f"Part One: {sum_part}")

# Now we search through the gear map to add the product of the adjacent parts
sum_gear = 0
for gear in gear_map.values():
  # Only when there's just 2 parts
  if( len(gear) == 2 ):
    sum_gear += (gear[0]*gear[1])
    
print(f"Part Two: {sum_gear}")