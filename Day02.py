...
id_sum = 0    # Answer for Part One (sum of possible game ids)
power_sum = 0 # Answer for Part Two (sum of "powers")

for line in input.splitlines():
  # First we separate the "Game ##" from subsets
  game_id, subsets = line.split(": ")
  
  # We need the number part
  id = int(game_id[5:])
  
  # Now we separete the subsets in groups of "grabs"
  grab_list = subsets.split(";")
  
  # This flag is used only in Part One
  impossible = False
  
  # These values are used only in Part Two
  max_red = max_green = max_blue = 0
  
  # Let's see how are these grabs
  for grab in grab_list:
    ...
    for c in colors:
      c = c.strip()
      if( c.find("red") >= 0 ):
        red = int(c.replace("red", ""))
      elif( c.find("green") >= 0):
        green = int(c.replace("green", ""))
      elif( c.find("blue") >= 0):
        blue = int(c.replace("blue", ""))
    # Updating these max values for each color
    max_red = max(max_red, red)
    max_green = max(max_green, green)
    max_blue = max(max_blue, blue)
    
    # Maximum: 12 red, 13 green, 14 blue for Part One
    if( red > 12 or green > 13 or blue > 14 ):
      impossible = True
  
  # If the game is not impossible, add it's id to the sum
  if not impossible:
    id_sum = id_sum + id
    
  # Calculating the "power" of this game and adding to the sum
  power = max_red * max_green * max_blue  
  power_sum = power_sum + power
    
# Printing the results
print(f"Part One: {id_sum}")
